...
\label{kernel}
Belief contraction is the process of removing beliefs from belief sets. It can be done by either of two ways. The first way is selecting one of the largest subsets of the belief set that do not logically imply the belief to be removed. This is called the \textit{remainder set} approach. It works by finding the largest subsets of the belief set, such that those subsets do not imply the removed belief. For example, the knowledge base:
\begin{center}
...
\end{center}
And they are the largest such subsets. This means that there are no larger subset of the original set that do not imply $b$. So we can choose one of them to be the new (contracted) belief set.
