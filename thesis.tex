...
\begin{abstract}
Kernel contraction is an interesting problem that can be used as a step towards belief revision. Kernels were introduced to determine why a given belief is accepted by the knowledge base. The aim of using kernels is to invalidate the reasons why that given belief is accepted, and hence rejecting it. In this study we will explain what kernels are and how we can use them to contract beliefs in an $\mathcal{EL}$ TBox. We will introduce some algorithms that performs belief contraction in different ways. We will also see how to determine the rationality of belief contraction by the rules introduced in the AGM framework.
\end{abstract}
